...
* https://github.com/ValveSoftware/halflife/blob/master/dlls/python.cpp
*
* Modified for the Crack-Life conversion
*
...
{
	// Models
	const string P_MDL = "models/p_357.mdl";
	const string V_MDL = "models/hldm-br/cracklife/v_357.mdl";
	const string W_MDL = "models/w_357.mdl";

	// Sounds
	const string S_FIRE1 = "hldm-br/cracklife/weapons/357_shot1.wav"; //POW
	//const string S_FIRE2 = "hldm-br/cracklife/weapons/357_shot2.wav";
	const string S_EMPTY = "hldm-br/cracklife/weapons/357_cock1.wav"; //NO!

	// Weapon Info
	const uint MAX_AMMO = 36;
	const uint MAX_CLIP = 6;
	const uint WEIGHT = 15;
	const uint DAMAGE = 0; // 0 = default
	const uint FOV = 20; // 40 HL default (fov 90)
	...
	class weapon_clpython : ScriptBasePlayerWeaponEntity
	{
		...
		bool Deploy()
		{
			return self.DefaultDeploy( V_MDL, P_MDL, CLPYTHON_DRAW, "python" );
		}
		...
		void SecondaryAttack()
		{
			...
			else if( self.m_fInZoom == false ) // m_pPlayer.pev.fov != FOV
			{
				self.m_fInZoom = true;
		...
		void PrimaryAttack()
		{
			...
			g_SoundSystem.EmitSoundDyn( m_pPlayer.edict(), CHAN_WEAPON, S_FIRE1, Math.RandomFloat( 0.92, 1.0 ), ATTN_NORM, 0, 98 + Math.RandomLong( 0, 3 ) );
			...
			m_pPlayer.FireBullets( 1, vecSrc, vecAiming, VECTOR_CONE_1DEGREES, 8192, BULLET_PLAYER_357, 0, DAMAGE, m_pPlayer.pev );
		...
		void Reload()
		{
			...
			self.DefaultReload( MAX_CLIP, CLPYTHON_RELOAD, 2.0, 0 );
			BaseClass.Reload();
			self.m_flTimeWeaponIdle = WeaponTimeBase() + g_PlayerFuncs.SharedRandomFloat( m_pPlayer.random_seed,  10, 15 );
		}
